...
remove features that disturb the generalization
"""
import numpy as np
import pandas as pd
...
if __name__ == '__main__':
            ...
            for train_index ,test_index in skf.split(x,y):
                ...
                for study_test in Study:
                    if study_test == study:
                        ...
                        train label = 1
                        test label = -1
                        train a model to classify the train data and test data
                        """
                        train_label = np.ones(shape=(len(train_x),1))
                        test_label = -np.ones(shape=(len(test_stst_x),1))
                        train = np.concatenate((train_x,test_stst_x),axis=0)
                        label = np.concatenate((train_label,test_label),axis=0)
                        data = np.concatenate((train,label),axis=1)
                        np.random.shuffle(data)
                        train = data[:,0:len(train_x[0])]
                        label = data[:,-1]
                        ...
                        score = lr.score(test_removed,test_stst_y)
    ...
    for study in Study:
        ...
        for i in range(10):
            ...
            for train_index, test_index in skf.split(x, y):
                ...
                train_label = np.ones(shape=(len(train_x), 1))
                test_label = -np.ones(shape=(len(test_loso_x), 1))
                train = np.concatenate((train_x, test_loso_x), axis=0)
                label = np.concatenate((train_label, test_label), axis=0)
                data = np.concatenate((train, label), axis=1)
                np.random.shuffle(data)
                train = data[:, 0:len(train_x[0])]
    ...
    accuracy_matrix.to_csv('accuracy_matrix_features_removed.csv')