...
if __name__ == '__main__':
            ...
            for train_index ,test_index in skf.split(x,y):
                ...
                for study_test in Study:
                    if study_test == study:
                        ...
                        weighted = get_beta(train_x,test_stst_x)
                        lr = LogisticRegression(penalty='l1',solver='liblinear',n_jobs=-1,C=parameters_stst[study])
                        lr.fit(train_x,train_y,sample_weight=weighted)
    ...
    for study in Study:
        ...
        for i in range(10):
            ...
            for train_index, test_index in skf.split(x, y):
                ...
                weighted = get_beta(train_x,test_loso_x)
                lr = LogisticRegression(penalty='l1', solver='liblinear', n_jobs=-1,C = parameters_loso[study])
                lr.fit(train_x,train_y,sample_weight=weighted)
    ...
    predict_matrix.to_csv('predict_matrix_KMM.csv')
    accuracy_matrix.to_csv('accuracy_matrix_KMM.csv')