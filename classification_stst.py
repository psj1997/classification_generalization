...
if __name__ == '__main__':
    ...
    accuracy_matrix_stst = {'FR-CRC': {'FR-CRC': [], 'AT-CRC': [], 'CN-CRC': [], 'US-CRC': [], 'DE-CRC': []},
                       'AT-CRC': {'FR-CRC': [], 'AT-CRC': [], 'CN-CRC': [], 'US-CRC': [], 'DE-CRC': []},
                       'CN-CRC': {'FR-CRC': [], 'AT-CRC': [], 'CN-CRC': [], 'US-CRC': [], 'DE-CRC': []},
                       'US-CRC': {'FR-CRC': [], 'AT-CRC': [], 'CN-CRC': [], 'US-CRC': [], 'DE-CRC': []},
                       'DE-CRC': {'FR-CRC': [], 'AT-CRC': [], 'CN-CRC': [], 'US-CRC': [], 'DE-CRC': []}}
    accuracy_matrix_loso = {'FR-CRC': [], 'AT-CRC': [], 'CN-CRC': [], 'US-CRC': [], 'DE-CRC': []}



    accuracy_matrix = pd.DataFrame(np.zeros(shape=(5,6)),index = ['FR-CRC', 'AT-CRC', 'CN-CRC', 'US-CRC', 'DE-CRC']
    ...
    #for study in ['AT-CRC']:
        ...
        x = x.values
        # std_preprocess = np.std(x,axis=0)
        # x = x[:,std_preprocess!=0]
        # x = np.log10(x + log_n0)
        ...
        # parameters = {'C': [0.1, 0.5, 1, 5, 10, 100,1000]}
        # gs = GridSearchCV(LogisticRegression(), parameters, refit=True, cv=10, n_jobs=-1)
        # gs.fit(x, y)
    ...
    for study in Study:
        ...
        # x = (x - mean) / (std + q)
        y =y.values
        # parameters = {'C': [ 0.1, 0.5, 1, 5, 10, 100]}
        # gs = GridSearchCV(LogisticRegression(), parameters, refit=True, cv=10, verbose=1, n_jobs=-1)
        # gs.fit(x, y)
        ...
        for i in range(10):
            ...
            for train_index, test_index in skf.split(x, y):
                ...
                accuracy_matrix.loc[study,'LOSO'] += score / 100
    ...
    # for study in Study:
    #     for study1 in Study:
    #         print('Training:{}  '.format(study),'Testing:{}  '.format(study1),'Accuracy:',np.array(accuracy_matrix_stst[study][study1]).mean())
    #
    # for study in Study:
    #     print('Testing:{}  '.format(study),'Accuracy:',np.array(accuracy_matrix_loso[study]).mean())
